package com.robotrade.robotradeapi.rabbitMQ.constants;

public class TransactionConstants {
	public static final String TRANSACTION_ROUTING_KEY = "roboTrade.transaction";
	public static final String TRANSACTION_HISTORY_ROUTING_KEY = "roboTrade.transactionHistory";
	public static final String ROBO_TOPIC_EXCHANGE_NAME = "roboExchange";
	public static final String ROBO_TRANSACTION_QUEUE_NAME = "roboTransactionQueue";
}
